...
class NotificationRoutes(
    ...
    private val notificationLogRepository: Flow<NotificationLogRepository>? = null,
) {
    ...
    @Serializable
    private data class WebhookVerifyRequest(val url: String)

    @Serializable
    private data class WebhookVerifyResponse(val valid: Boolean, val name: String? = null, val error: String? = null)

    private fun Route.discordVerifyRoute() {
        post("/verify") {
            val request = call.receive<WebhookVerifyRequest>()
            ...
            if (!url.startsWith("https://discord.com/api/webhooks/") &&
                ...
                call.respond(
                    HttpStatusCode.OK,
                    WebhookVerifyResponse(valid = false, error = "Not a valid Discord webhook URL")
                )
                return@post
            }
            ...
            try {
                ...
                if (response.isSuccessful) {
                    ...
                    // Extract webhook name from JSON response
                    val nameMatch = Regex("\"name\"\\s*:\\s*\"([^\"]+)\"").find(body)
                    call.respond(
                        HttpStatusCode.OK,
                        WebhookVerifyResponse(valid = true, name = nameMatch?.groupValues?.get(1))
                    )
                } else {
                    call.respond(
                        HttpStatusCode.OK,
                        WebhookVerifyResponse(valid = false, error = "Discord returned ${response.code}")
                    )
                }
            } catch (e: Exception) {
                call.respond(
                    HttpStatusCode.OK,
                    WebhookVerifyResponse(valid = false, error = e.message ?: "Connection failed")
                )
            }
        }
    }
    ...
    @Serializable
    private data class NotificationLogResponse(
        val id: Long,
        val jobId: String?,
        val channel: String,
        val url: String,
    ...
    @Serializable
    private data class NotificationLogsPage(
        val logs: List<NotificationLogResponse>,
        val totalCount: Long,
    )

    private fun Route.notificationLogsRoute() {
        get("/logs") {
            val repo = notificationLogRepository?.first()
            if (repo == null) {
                call.respond(HttpStatusCode.OK, NotificationLogsPage(emptyList(), 0))
                return@get
            }
            val limit = (call.request.queryParameters["limit"]?.toLongOrNull() ?: 50).coerceIn(1, 200)
            val offset = (call.request.queryParameters["offset"]?.toLongOrNull() ?: 0).coerceAtLeast(0)
            val logs = repo.findAll(limit, offset)
            val totalCount = repo.countAll()
            call.respond(
                HttpStatusCode.OK,
                NotificationLogsPage(
                    logs = logs.map {
                        NotificationLogResponse(
                            ...
                            sentAt = it.sentAt.toEpochMilliseconds(),
                        )
                    },
