...
import snd.komf.api.komga.KomgaIntegrationStatus
...
class MetadataRoutes(
    ...
    private val appConfig: Flow<snd.komf.app.config.AppConfig>,
) {
    ...
    @Serializable
    private data class BulkMatchRequest(val seriesIds: List<String>)

    @Serializable
    private data class BulkMatchResponse(val jobIds: List<String>)

    private fun Route.matchBulkRoute() {
        post("/match-bulk") {
            val request = call.receive<BulkMatchRequest>()
            if (request.seriesIds.isEmpty()) {
                return@post call.respond(HttpStatusCode.BadRequest, KomfErrorResponse("seriesIds must not be empty"))
            }

            val jobIds = request.seriesIds.map { seriesIdStr ->
                val seriesId = MediaServerSeriesId(seriesIdStr)
                val series = mediaServerClient.first().getSeries(seriesId)
                val jobId = metadataServiceProvider.first()
                    .metadataServiceFor(series.libraryId.value)
                    .matchSeriesMetadata(seriesId)
                jobId.value.toString()
            }

            call.respond(BulkMatchResponse(jobIds))
        }
    }
    ...
    private fun Route.integrationStatusRoute() {
        get("/integration/status") {
            try {
                val config = appConfig.first()
                val mediaServer = mediaServerClient.first()
                val connected = mediaServer.getLibraries().isNotEmpty()
                call.respond(
                    KomgaIntegrationStatus(
                        connected = connected,
                        baseUri = config.komga.baseUri
                    )
                )
            } catch (e: Exception) {
                logger.catching(e)
                call.respond(
                    KomgaIntegrationStatus(
                        connected = false,
                        baseUri = appConfig.first().komga.baseUri,
