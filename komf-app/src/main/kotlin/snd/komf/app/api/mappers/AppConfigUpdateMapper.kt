...
import snd.komf.api.config.AutoDownloaderConfigUpdateRequest
import snd.komf.api.config.BookMetadataConfigUpdateRequest
import snd.komf.api.config.DiscordConfigUpdateRequest
import snd.komf.api.config.DownloadConfigUpdateRequest
...
import snd.komf.api.config.SchedulerConfigUpdateRequest
...
class AppConfigUpdateMapper {
    ...
    fun patch(config: AppConfig, patch: KomfConfigUpdateRequest): AppConfig {
        return config.copy(
            ...
            scheduler = patch.scheduler.getOrNull()
                ?.let { schedulerConfig(config.scheduler, it) }
                ?: config.scheduler,
            download = patch.download.getOrNull()
                ?.let { downloadConfig(config.download, it) }
                ?: config.download,
            autoDownloader = patch.autoDownloader.getOrNull()
                ?.let { autoDownloaderConfig(config.autoDownloader, it) }
                ?: config.autoDownloader,
        )
    }
    ...
    private fun schedulerConfig(config: SchedulerConfig, patch: SchedulerConfigUpdateRequest): SchedulerConfig {
        return config.copy(
            autoMatchEnabled = patch.autoMatchEnabled.getOrNull() ?: config.autoMatchEnabled,
            autoMatchIntervalHours = patch.autoMatchIntervalHours.getOrNull() ?: config.autoMatchIntervalHours,
        )
    }

    private fun downloadConfig(config: DownloadConfig, patch: DownloadConfigUpdateRequest): DownloadConfig {
        return config.copy(
            downloadDir = patch.downloadDir.getOrNull() ?: config.downloadDir,
            komgaLibraryId = when (val v = patch.komgaLibraryId) {
                PatchValue.None -> null
                is PatchValue.Some -> v.value
                PatchValue.Unset -> config.komgaLibraryId
            },
            komgaLibraryPath = when (val v = patch.komgaLibraryPath) {
                PatchValue.None -> null
                is PatchValue.Some -> v.value
                PatchValue.Unset -> config.komgaLibraryPath
            },
    ...
    private fun autoDownloaderConfig(config: AutoDownloaderConfig, patch: AutoDownloaderConfigUpdateRequest): AutoDownloaderConfig {
        return config.copy(
            enabled = patch.enabled.getOrNull() ?: config.enabled,
            intervalHours = patch.intervalHours.getOrNull() ?: config.intervalHours,
            notifyOnNewChapters = patch.notifyOnNewChapters.getOrNull() ?: config.notifyOnNewChapters,
        )
    }
}