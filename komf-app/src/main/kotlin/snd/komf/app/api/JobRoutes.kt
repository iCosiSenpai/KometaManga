...
class JobRoutes(
    ...
    private val metadataServiceProvider: Flow<MetadataServiceProvider>? = null,
    private val mediaServerClient: Flow<MediaServerClient>? = null,
) {
    fun registerRoutes(routing: Route) {
        routing.route("/jobs") {
            ...
            deleteAllRoute()
            cancelJobRoute()
            if (metadataServiceProvider != null && mediaServerClient != null) {
                retryJobRoute()
            }
        }
    }
    ...
    private fun Route.cancelJobRoute() {
        delete("/{jobId}") {
            val jobId = UUID.fromString(call.parameters.getOrFail("jobId"))
            val cancelled = jobTracker.first().cancelMetadataJob(MetadataJobId(jobId))
            if (cancelled) {
                call.respond(HttpStatusCode.NoContent, "")
            } else {
    ...
    private fun Route.retryJobRoute() {
        post("/{jobId}/retry") {
            val jobId = UUID.fromString(call.parameters.getOrFail("jobId"))
            val job = jobsRepository.first().get(MetadataJobId(jobId))
                ?: return@post call.respond(HttpStatusCode.NotFound, "")

            if (job.status == MetadataJobStatus.RUNNING) {
                return@post call.respond(HttpStatusCode.Conflict, "")
            }

            val series = mediaServerClient!!.first().getSeries(job.seriesId)
            val newJobId = metadataServiceProvider!!.first()
                .metadataServiceFor(series.libraryId.value)
                .matchSeriesMetadata(job.seriesId)

            call.respond(
                KomfMetadataJobId(newJobId.value.toString())
            )
        }
    }
