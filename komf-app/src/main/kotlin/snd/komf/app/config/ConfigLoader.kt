...
class ConfigLoader(private val yaml: Yaml) {
    ...
    fun loadDirectory(directory: Path): AppConfig {
        ...
        val config = if (path.isReadable()) {
            yaml.decodeFromString(AppConfig.serializer(), Files.readString(path))
        } else {
            logger.warn { "Config file not found at $path, using defaults and environment variables" }
            AppConfig()
        }
    ...
    private fun overrideConfigDirAndEnvVars(config: AppConfig, configDirectory: String?): AppConfig {
        ...
        val downloadConfig = config.download
        val downloadDir = System.getenv("KOMF_DOWNLOAD_DIR")?.ifBlank { null } ?: downloadConfig.downloadDir
        val downloadCbzCompression = System.getenv("KOMF_DOWNLOAD_CBZ_COMPRESSION")?.toBooleanStrictOrNull()
            ?: downloadConfig.cbzCompression
        val downloadConcurrentPages = System.getenv("KOMF_DOWNLOAD_CONCURRENT_PAGES")?.toIntOrNull()
            ?: downloadConfig.concurrentPageDownloads
        val downloadAutoScan = System.getenv("KOMF_DOWNLOAD_AUTO_SCAN")?.toBooleanStrictOrNull()
            ?: downloadConfig.autoScanAfterDownload

        val autoDownloaderConfig = config.autoDownloader
        val autoDownloaderEnabled = System.getenv("KOMF_AUTO_DOWNLOADER_ENABLED")?.toBooleanStrictOrNull()
            ?: autoDownloaderConfig.enabled
        val autoDownloaderInterval = System.getenv("KOMF_AUTO_DOWNLOADER_INTERVAL")?.toIntOrNull()
            ?: autoDownloaderConfig.intervalHours

        val schedulerConfig = config.scheduler
        val schedulerAutoMatchEnabled = System.getenv("KOMF_SCHEDULER_AUTO_MATCH_ENABLED")?.toBooleanStrictOrNull()
            ?: schedulerConfig.autoMatchEnabled
        val schedulerAutoMatchInterval = System.getenv("KOMF_SCHEDULER_AUTO_MATCH_INTERVAL")?.toIntOrNull()
            ?: schedulerConfig.autoMatchIntervalHours
        ...
        return config.copy(
            ...
            logLevel = logLevel,
            download = downloadConfig.copy(
                downloadDir = downloadDir,
                cbzCompression = downloadCbzCompression,
                concurrentPageDownloads = downloadConcurrentPages,
                autoScanAfterDownload = downloadAutoScan
            ),
            autoDownloader = autoDownloaderConfig.copy(
                enabled = autoDownloaderEnabled,
                intervalHours = autoDownloaderInterval
            ),
            scheduler = schedulerConfig.copy(
                autoMatchEnabled = schedulerAutoMatchEnabled,
                autoMatchIntervalHours = schedulerAutoMatchInterval
            )
        )
    }
