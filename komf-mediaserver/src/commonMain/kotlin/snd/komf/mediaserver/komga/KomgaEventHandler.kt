...
class KomgaEventHandler(
    ...
    fun start() {
        eventHandlerScope.launch {
            while (eventHandlerScope.isActive) {
                try {
                    logger.info { "Connecting to Komga events stream" }
                    ...
                    logger.warn(e) { "Komga events stream disconnected. Reconnecting in ${reconnectDelayMs}ms" }
