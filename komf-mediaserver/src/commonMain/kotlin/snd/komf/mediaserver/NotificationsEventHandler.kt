    ...
    override suspend fun onBooksAdded(events: List<BookEvent>) {
        events.filter { libraryFilter.test(it.libraryId.value) }
            ...
            .mapNotNull { (seriesId, events) ->
                ...
                runCatching { discordWebhookService.send(context) }
                    .onSuccess { notificationLogRepository?.logSuccess("discord", "webhook") }
                    .onFailure { e ->
                        logger.catching(e)
                        notificationLogRepository?.logFailure("discord", "webhook", e.message ?: "Unknown error")
                    }
                runCatching { appriseService.send(context) }
                    .onSuccess { notificationLogRepository?.logSuccess("apprise", "cli") }
                    .onFailure { e ->
                        logger.catching(e)
                        notificationLogRepository?.logFailure("apprise", "cli", e.message ?: "Unknown error")
                    }
            }
    }
